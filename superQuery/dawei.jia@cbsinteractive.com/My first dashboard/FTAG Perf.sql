SELECT day_dt, replace(ftag,"ftag:","") as ftag, count(distinct visit_session_id) as visits, count(distinct post_visitor_id) as devices,
       COUNT(distinct case when post_evar10_desc ="svod_complete" then post_visitor_id end) as orders,
       COUNT(case when post_evar52_desc is not null then post_visitor_id end) as streams
